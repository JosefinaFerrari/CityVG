...
def get_itinerary(request):
    ...
    try:
        ...
        start_date = datetime.strptime(
            request.GET.get("start_date"), "%Y-%m-%dT%H:%M:%S"
        )
        ...
        for attraction in itinerary["attractions"]:
            ...
            if name in merged_places_x_tiqets:
                ...
                attraction.update(
                    {
                        ...
                        "product name": merged_places_x_tiqets[name]["products"][
                            list(merged_places_x_tiqets[name]["products"].keys())[0]
                        ]["title"],
                        "price": merged_places_x_tiqets[name]["products"][
                            list(merged_places_x_tiqets[name]["products"].keys())[0]
                        ]["price"],
                        "checkout url": url,
                    }
                ...
                if "productName" in attraction:
                    ...
                    product_info = merged_places_x_tiqets[name]["products"].get(
                        product_name
                    )
...
def restructure_products(merged_data):
    ...
    for place_name, place_info in merged_data.get("products", {}).items():
        # Extract the inner products dictionary
        place_products = place_info.get("products", {})

        # Merge the products into the flattened_products dictionary
...
def get_recommendations(request):
    ...
    latt = float(request.GET.get("lat", 0))
    lngg = float(request.GET.get("lng", 0))
    radiuss = int(request.GET.get("radius", 5000))
    # Fetch data from external sources
    places_data = get_places(latt, lngg, radiuss, categories).get("places", [])
    tiqets_data = get_tiqets_products(latt, lngg, radiuss).get("products", [])
    merged_dataa = merge_places_tiqets(places_data, tiqets_data)
